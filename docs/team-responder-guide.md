...
## 참가자 안내 팁: Discord에서 내 질문/답변만 보기
질문이 많이 올라와서 자기 답변을 놓칠 수 있음. 참가자에게 안내:
- 화면 우측 상단 **@ (멘션) 아이콘** 클릭 → "받은 편지함" 탭
- 나에게 온 멘션 + 내가 멘션된 답변만 모아서 표시
- 또는 검색창에서 `from: JARVIS-W mentions: 내닉네임` 으로 필터링
